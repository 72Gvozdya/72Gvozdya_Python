    ...
    with open(file) as f:
       # match = re.finditer(regex, f.read())
        ...
        for line in f:
            ...
            if match and match.group('intf'):
                intf_name = match.group('intf')
                groups_list = []
            elif match and match.group('ip') and match.group('mask'):
                groups_list.append(match.group('ip', 'mask'))
                result[intf_name] = groups_list
            else: pass
