...
def generate_description_from_cdp(sh_cdp_filename):
    regex = r'(?P<dev>\S\d) +(?P<intf>\S+ \S+) .+(?P<port>Eth \S+)'
    result = []
    template = "description Connected to {dev} port {port}"
    with open(sh_cdp_filename) as f:
        match = re.finditer(regex, f.read())
        result = {m.group('intf'): template.format(**m.groupdict()) for m in match}
        return result
