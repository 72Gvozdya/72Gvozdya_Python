...
def parse_sh_version(command):
    result = ()
    regex = (r'Version (?P<ver>\S+),.+?uptime is (?P<uptime>\d+ days, \d+ hours, \d+ minutes)'
            r'.+?image file is \"(?P<img>\S+)\"')
    match = re.search(regex, command, re.DOTALL)
    if match:
        result = match.group('ver', 'img', 'uptime')
    return result
...
def write_inventory_to_csv(data_filenames, csv_filename):
    with open(csv_filename, 'w') as dest:
        ...
        for filename in data_filenames:
            host = re.search(r'sh_version_(\w+)', filename).group(1)
            with open(filename) as f:
                writer.writerow((host,) + parse_sh_version(f.read()))

if __name__ == '__main__':
    write_inventory_to_csv(sh_version_files, 'task_17_2.csv')

with open('task_17_2.csv') as ff:
    print(ff.read())